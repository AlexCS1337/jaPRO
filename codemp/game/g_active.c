...
//
	...
	if (ent && ent->client && ent->client->sess.raceMode) {
		...
		if (movementStyle == MV_RJCPM || movementStyle == MV_RJQ3) {
			...
			ent->client->ps.fd.forcePowerLevel[FP_LIGHTNING] = ent->client->ps.fd.forcePowerLevel[FP_LIGHTNING] = 2;
			ent->client->ps.fd.forcePowerLevel[FP_SPEED] = ent->client->ps.fd.forcePowerLevel[FP_GRIP] = ent->client->ps.fd.forcePowerLevel[FP_LIGHTNING] = 
